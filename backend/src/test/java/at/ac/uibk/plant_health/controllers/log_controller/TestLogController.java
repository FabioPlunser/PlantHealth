    ...
    public void singleLog() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("/get-logs")
                ...
                .andExpectAll(
                        status().isOk(),
                        jsonPath("$.logs").isArray(),
                        jsonPath("$.logs[0]").exists(),
                        jsonPath("$.logs[1]").doesNotExist(),

                        jsonPath("$.logs[0].severity").value(Matchers.equalTo(log.getSeverity().toString())),
                        jsonPath("$.logs[0].timestamp").value(
                                new LocalDateTimeJsonParser(log.getTimeStamp(), Duration.ofSeconds(TIME_TOLERANCE))
                        ),
