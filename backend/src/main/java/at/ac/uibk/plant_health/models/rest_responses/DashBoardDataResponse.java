...
public class DashBoardDataResponse extends RestResponse implements Serializable {
    ...
    public DashBoardDataResponse(Person person) {
        this.plants = person.getSensorStationPersonReferences().stream()
                .map(SensorStationPersonReference::getSensorStation)
                .map(DashboardPlant::new).toList();
    }
    ...
    private class DashboardPlant {
        ...
        public DashboardPlant(SensorStation sensorStation) {
            ...
            this.values = sensorStation.getSensorData().stream().collect(groupingBy(SensorData::getTimeStamp))
                    .entrySet().stream().map(TimeStampedSensorData::new).sorted(Comparator.comparing(o -> o.timestamp)).toList();
        }
    }
