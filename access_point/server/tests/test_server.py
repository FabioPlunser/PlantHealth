...
def test_register(requests_mock: Mocker):
    ...
    backend = Server(URL)
    token = str(uuid4())
    requests_mock.post(f'{URL}/register-access-point',
...
def test_register_declined(requests_mock: Mocker):
    ...
    backend = Server(URL)
    requests_mock.post(f'{URL}/register-access-point', status_code=401)
    returned_token = backend.register('id', 'room')
...
def test_get_config(requests_mock: Mocker):
    ...
    requests_mock.get(f'{URL}/get-access-point-config',
...
def test_revoke_token_with_get_config(requests_mock: Mocker):
    ...
    """
    backend = Server(URL, TOKEN)
    requests_mock.get(f'{URL}/get-access-point-config', status_code=401)
...
def test_transfer_data(requests_mock: Mocker):
    ...
    assert sent[0]['sensorData'][1]['timeStamp'] == timestamp.isoformat(timespec="seconds")
